...
All three tiers share the exact same product, initial inventory, demand process, and replenishment plan *timing and mean quantity* (V2.3.5) — **only the network's nodes and edges, plus one deliberate exception, differ between tiers.** The exception: Standard's release quantity stays fixed (`shipment_quantity_std: 0`, `minimum`/`maximum_shipment_quantity: 40`) rather than drawing from the same distribution Compact and Extended use (`std: 6`, `min: 20`, `max: 55`) — this is intentional, not an oversight, and is pinned by a dedicated regression test (`test_zero_std_reproduces_fixed_quantity`, `tests/unit/test_loaders.py`): it keeps Standard × Medium a true behavioral anchor to V1's original audited 100-replication run (V1's own shipments really were always exactly 40 units), the same non-negotiable reason Standard's nodes/edges stay byte-identical. `test_extended_and_compact_replenishment_plan_matches_standard` reflects this on purpose — it asserts Compact/Extended match Standard's `shipment_quantity_mean` only, deliberately not `std`/`min`/`max`. The practical consequence: a Standard-vs-Compact/Extended comparison is not fully isolated from a shipment-volume confound the way a same-topology, same-severity comparison is — Standard's replications see zero quantity variance where Compact/Extended's see real spread. Compact and Extended do fully share one template with each other.
...
- `release_shipments`'s return type changes from `None` to `int`: the quantity actually released this call (today's newly-succeeded releases plus any successfully-retried pending ones). This is required by the `simulation/engine.py` correction directly below — without it, `total_released` cannot stay correct once a release can defer instead of always succeeding.

### `simulation/engine.py` — one narrow correction to this section's original "no changes" claim

V2.3.7's claim that the product-balance invariant holds "without modification" assumed `engine.py` would not need to change. That assumption was wrong: `engine.py`'s `run()` computed `total_released` by summing every scheduled release event's quantity unconditionally, *before* calling `release_shipments`, relying on V1's guarantee that a release always either succeeds or raises `SimulationInvariantError`. Once a release can silently defer (V2.3.7), that unconditional sum overcounts on the day of a deferral and the balance assertion in `_assert_invariants` breaks. The fix, confirmed with Yassine during the V2 Milestone 10–13 build pass: `_process_day` captures `release_shipments`'s returned actually-released quantity and threads it through as the new `total_released` (replacing the old unconditional day-event sum), returning it alongside `metrics` and `delivered_count` so `run()` can carry it to the next day. This is a narrow, mechanical bookkeeping change — it does not alter the daily step order, branch on policy type, or touch anything else in this file. `simulation/engine.py` should therefore be read out of "Everything else" below and understood to have exactly this one change.
...
**Implemented (Milestone 15, done).** Gains the ability to accept the 2D (topology × severity) grid via one or more `--cell TOPOLOGY SEVERITY PATH` triples (repeatable, 0–9 times; the grid need not be complete — missing combinations render as labeled blanks rather than being silently treated as zero or omitted). Each `--cell` also gets the normal per-scenario plot set (labeled `"<Topology> x <Severity>"`), exactly like a matched `--experiment`/`--label` pair. `--experiment`/`--label` are no longer individually `required` at the argparse level — the script now requires at least one `--experiment` or one `--cell` — since a grid-only invocation is a legitimate, common case.

Giving ≥1 `--cell` additionally produces six grid-level plots into `<output-dir>/grid/`, reclaiming the `08_disruption_robustness_heatmap.png` filename V1's version of this file had reserved (and explicitly skipped) for exactly this purpose:

- **08** — mean-delta heatmap, diverging color (orange = LLM cheaper, blue = heuristic cheaper, matching every other plot's sign convention), hatched overlay on any cell whose 95% CI on mean delta includes zero (not distinguishable from a tie at that cell's replication count — expected and common on 3-replication calibration data, material once the full 33-rep grid runs).
- **11** — companion LLM win-rate heatmap (magnitude and frequency are different questions; kept as two figures rather than double-encoding one heatmap).
- **12** — topology × severity interaction plot (mean delta vs. severity, one line per topology tier, 95% CI ribbons) — the direct visual answer to V2.1's mission-delta question ("how does that advantage vary with network complexity and disruption character").
- **13** — cross-cell win/loss/tie stacked bars, fixed topology-then-severity order.
- **14** — signal-to-noise ratio (`|mean delta| / stdev(delta)`) per cell, plotted against the same ratio computed fresh from whichever real V1 100-replication runs are passed via the new, optional, repeatable `--v1-reference PATH` flag — the direct, self-computing check on whether V2's redesign actually fixed the audit finding that motivated it (V1's ratio was 7.6×–10.2× in every profile), rather than a number quoted from this file's own prose.
- **15** — a QA/trust plot (not a results plot) confirming the realized shock timing/duration/information-delay and release-quantity randomness actually has spread, streamed from each cell's `event_tapes.jsonl`; one timing sample per replication is taken from that replication's lowest-`shock_id` shock, so a multi-shock scenario's distinct templates are never pooled into one misleading distribution.

The existing per-experiment and cross-severity plots (01–07, 09, 10, built for V1) are unchanged. Because `analysis/` sits outside `src/supply_chain_simulator/` and outside mypy/pytest scope (V1 §4, confirmed unchanged by V2.2), this addition carries no new testing or type-checking obligation beyond what V1 already established for this script; it was, however, run against real Milestone-14 calibration output for all nine cells plus the three real V1 100-replication runs as `--v1-reference`, confirmed error-free, and each new plot was visually inspected.

**A finding surfaced while building plot 15, since resolved:** `configs/networks/baseline_network.yaml`'s replenishment plan has `shipment_quantity_std: 0` (still fully fixed at exactly 40 units, V1-identical) while `topology_compact.yaml`/`topology_extended.yaml` have `shipment_quantity_std: 6` (real spread per V2.3.5) — initially flagged here as a possible contradiction of V2.3.1's "all three tiers share the exact same ... replenishment plan template." Checking `tests/unit/test_loaders.py` (committed as part of the original Milestone 10–13 work) showed this is deliberate and already tested: `test_zero_std_reproduces_fixed_quantity` pins Standard's fixed quantity, and `test_extended_and_compact_replenishment_plan_matches_standard` was already written to check only `shipment_quantity_mean` equality across tiers, not `std`/`min`/`max`. Confirmed with Yassine: the fix was to V2.3.1's prose (now corrected there), not the config — Standard's zero-variance quantity stays, as an intentional behavioral anchor to V1's audited run.

### Everything else

`simulation/routing.py`, `simulation/costs.py`, `decisions/observation.py`, `decisions/validator.py`, `policies/*.py`, `integrations/llm_client.py`, `data_io/writers.py`, `cli.py` — **no changes**. (`simulation/engine.py` has exactly the one correction described in its own entry above, and is not part of this "no changes" set.) V2's new randomness and new shock types resolve entirely to ordinary, already-supported domain objects (a realized `Shock`, a `ShipmentReleaseEvent` with a different quantity, a node's `source_capacity_multiplier`) before reaching any of these files; none of them can tell a V2-realized `Shock` apart from a V1 fixed one.
...
**Calibration done (2026-08-17); done when's `test_paired_experiment.py` per-tier check not separately confirmed here.** Write the nine `configs/experiments/*.yaml` grid cells (V2.8.1), run `validate-config` on all nine, then run a 3-replication real calibration per V2.8.2 to measure actual per-cell OpenAI spend at full horizon/drain before committing to the full grid. Done when: all nine configs validate; calibration produces a real, measured per-replication cost; `test_paired_experiment.py`'s per-tier full-run addition passes for at least one cell per topology tier. All nine configs exist and all nine have a completed real 3-replication calibration run under `outputs/*_calibration__20260817*` — this closes the "calibration has not run yet" gap V2.8.2's own prose still describes elsewhere in this document.

### Milestone 15 — Analysis grid heatmap

**Done (see `analysis/plot_results.py`'s file-contract entry above for the full list of what shipped).** Extend `analysis/plot_results.py` to accept the 2D grid and produce the disruption-robustness heatmap (V2.7). Done when: run against real Milestone-14 calibration output, the heatmap renders correctly with no errors, for at least a partial (non-9-cell) grid — confirmed against all nine real calibration output directories plus the three real V1 100-replication runs as `--v1-reference`.
