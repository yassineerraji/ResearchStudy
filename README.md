...
**In short: whether the AI agent helps depends on both how bad the disruption is and how much flexibility the network has to route around it.** It is not a universal win or a universal loss. The full numbers, caveats, and a discussion of what this does and doesn't prove are in [`report/Final_Report.pdf`](report/Final_Report.pdf) — see [Where to learn more](#where-to-learn-more).
...
report/         the write-up: the final LaTeX/PDF report (see below)
webapp/         optional, separate: a browsable front end over outputs/ and the CLI — see below
```

You will only ever need to touch files under `configs/` and the `.env` file described below — everything under `src/` is the program itself. `analysis/`, `report/`, and `webapp/` are all downstream of the simulator's output files, not part of the simulation logic.
...
The write-up built from this project's own real results — one combined V1+V2 LaTeX/PDF report — already exists under [`report/`](report/); see [Where to learn more](#where-to-learn-more).
...
|---|---|
| [`report/Final_Report.pdf`](report/Final_Report.pdf) | The full write-up, covering the *whole* study — V1 and V2 together as one two-phase research program: why it's built the way it is, the audit that motivated V2, and the full nine-cell grid results. |
| [`report/Final_Report.tex`](report/Final_Report.tex) | The LaTeX source for the above. |
