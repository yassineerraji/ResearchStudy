...
\textbf{Validation.} Standard~$\times$~Medium is Phase~1's original configuration, re-run unmodified at Phase 2 scale: LLM wins 100\% of 100 replications, matching Phase 1's original pattern almost exactly (compare Table~\ref{tab:v1}'s Medium row).
...
\label{eq:snr}
\end{equation}
and it sits at 0.6$\times$--3.0$\times$ across all nine Phase 2 cells, against the $7.6\times$--$10.2\times$ recomputed identically on Phase 1's real runs (Figure~\ref{fig:snr}): the audited root cause is measurably fixed.
...
Combining 798 replications across two phases and four network configurations, the study's answer to its research question is not universal but conditional.
...
Each of these is an extension to \emph{what the environment models}, in the same spirit as Phase 2's own topology and randomness extensions to Phase 1.
